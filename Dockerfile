...
# Ultimately, nginx is what allows us to serve our application to the user once live.
FROM nginx:alpine
...
# Copies the local app code to the container's web root where nginx will serve it from (where the docker container expects to find your app's code)
COPY . .
